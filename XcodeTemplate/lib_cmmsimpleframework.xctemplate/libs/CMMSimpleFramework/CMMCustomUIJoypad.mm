...
@synthesize pushDelayTime,autoPushdown;
...
-(void)update:(ccTime)dt_{
	...
	if(autoPushdown && _isOnPush && _curPushDelayTime>=pushDelayTime){
...
@synthesize stickVector,callback_whenStickVectorChanged;
...
-(void)setContentSize:(CGSize)contentSize{
	...
	_backSprite.position = ccp(_contentSize.width/2.0f,_contentSize.height/2.0f);
}
...
-(void)setStickVector:(CGPoint)stickVector_{
	float radius_ = _contentSize.width/2.0f;
	...
	if(!CGPointEqualToPoint(ccpSub(targetPoint_ , _stick.position) , CGPointZero)){
		if(callback_whenStickVectorChanged){
			callback_whenStickVectorChanged(stickVector_);
		}
	}
...
-(void)cleanup{
	[self setCallback_whenStickVectorChanged:nil];
	[super cleanup];
}
-(void)dealloc{
	[callback_whenStickVectorChanged release];
	[super dealloc];
}
...
-(void)resetJoypadPosition{
	...
	CGPoint buttonAPoint_ = ccp(_contentSize.width-170.0f,60.0f);
	CGPoint buttonBPoint_ = ccp(_contentSize.width-50.0f,100.0f);
...
-(BOOL)touchDispatcher:(CMMTouchDispatcher *)touchDispatcher_ shouldAllowTouch:(UITouch *)touch_ event:(UIEvent *)event_{
	return [CMMTouchUtil nodeCountInTouch:_children touch:touch_] && [super touchDispatcher:touchDispatcher_ shouldAllowTouch:touch_ event:event_];
}
